...
def send_input():
    ...
    agent = agents.get(unique_id)
    if agent is None:
        return jsonify({"error": "Invalid unique_id"})

    # Create a new thread to run the interaction loop in the background
    interaction_thread = threading.Thread(
        target=start_interaction_loop_background, args=(agent, user_input)
    )
    interaction_thread.start()

    # Immediately return a response to the API request
    return jsonify({"status": "interaction_started"})

def start_interaction_loop_background(agent, user_input):
    agent.start_interaction_loop(user_input=user_input)
